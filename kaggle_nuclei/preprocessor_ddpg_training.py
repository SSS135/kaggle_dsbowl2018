...
def train_preprocessor_gan(train_data, epochs=15, pretrain_epochs=7, affine_augmentation=False, resnet=False):
    ...
    if resnet:
        ...
        gen_model = UNet(3, 4).cuda()
        gen_optimizer = GAdam(get_param_groups(gen_model), lr=1e-4, betas=(0.5, 0.999), nesterov=0.0,
                              weight_decay=5e-5, avg_sq_mode='tensor', amsgrad=False)

    disc_model = LossNet(4).cuda()
    ...
    for epoch in range(epochs):
        with tqdm(dataloader) as pbar:
            ...
            for i, (img, mask, sdf) in enumerate(pbar):
                ...
                split_mask = split_labels(mask_train.data, mask_channels).float().clamp(min=0.05, max=0.95)
                split_mask += split_mask.clone().uniform_(-0.1, 0.1)
                ...
                out = gen_model(gen_input)[:, :, pad:-pad, pad:-pad]
                out_sdf, out_cont, out_mask = out[:, 0:1], out[:, 1:2], out[:, 2:]
                out_mask, out_sdf = F.sigmoid(out_mask), out_sdf.contiguous()

                fake_input = torch.cat([out_mask, sdf_train, cont_train, x_train_unpad], 1)
                ...
                loss = loss_gen + 0.5 * sdf_loss + 0.5 * cont_loss # sdf_loss.mean() + cont_loss.mean() #+ mask_loss
                loss.backward()
