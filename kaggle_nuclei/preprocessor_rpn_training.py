    ...
    for epoch in range(epochs):
        with tqdm(dataloader) as pbar:
            ...
            for batch_idx, data in enumerate(pbar):
                ...
                indices, target_masks, target_scores, img_crops, layer_idx = train_pairs
                pred_masks, pred_scores = get_masks_and_scores_from_indexes(model_gen, model_out_layers, indices)
...
def get_train_pairs(
    ...
    target_masks, target_scores, img_crops = [torch.cat(o, 0) for o in outputs]
    return indices, target_masks.unsqueeze(1).float(), target_scores, img_crops, layer_idx
...
def get_train_pairs_single(model, labels, sdf, img, net_out, pixel_sizes,
    ...
    for layer_idx, layer_data in enumerate(zip(net_out, resampled_layers)):
        ...
        if o is not None:
            outputs.append((*o, o[1].shape[0] * [layer_idx]))
    return outputs
...
def generate_samples_for_layer(model, out_masks, out_scores, labels, sdf, obj_boxes, img,
    ...
    target_scores = out_scores.data.new(len(pos_centers_fmap_idx) + len(neg_centers_fmap_idx)).fill_(0)
    target_scores[:len(pos_centers_fmap_idx)] = 1
    ...
    mask_score_indexes = pos_centers_fmap.shape[0], pos_centers_fmap_idx, neg_centers_fmap_idx

    return mask_score_indexes, target_masks, target_scores, img_crops
...
def get_masks_and_scores_from_indexes(model, net_out, indices):
    masks = []
    scores = []
    for si, sample_indices in enumerate(indices):
        if len(sample_indices) == 0:
            continue
        for li, layer_indices in enumerate(sample_indices):
            om, os = net_out[li]
            pm, ps = get_masks_and_scores_from_indexes_for_layer(model, om[si], os[si], layer_indices)
            masks.append(pm)
            scores.append(ps)
    masks, scores = [torch.cat(x, 0) for x in (masks, scores)]
    return masks, scores


def get_masks_and_scores_from_indexes_for_layer(model, out_masks, out_scores, indexes):
    pos_centers_fmap_size, pos_centers_fmap_idx, neg_centers_fmap_idx = indexes

    pred_pos_scores = out_scores.take(Variable(pos_centers_fmap_idx))
    pred_neg_scores = out_scores.take(Variable(neg_centers_fmap_idx))
    pred_scores = torch.cat([pred_pos_scores, pred_neg_scores])

    pred_masks = center_crop(out_masks, pos_centers_fmap_idx, (0, model.conv_size), pos_centers_fmap_size)
